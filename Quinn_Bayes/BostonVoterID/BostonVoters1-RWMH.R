...
w <- 0.15 ## width of uniform proposal 
...
for (iter in 1:tot.iter){
    ...
    ## generate candidate
    theta.can <- theta + runif(1, -w, w)

    ## calculate the acceptace probability
    log.numerator <- dbeta(theta.can, y+a, n-y+b, log=TRUE)
    log.denominator <- dbeta(theta, y+a, n-y+b, log=TRUE)
    ratio <- exp(log.numerator - log.denominator)
    u <- runif(1)
    if (u < ratio){
        theta <- theta.can
